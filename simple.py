...
def fig9():
    ...
    for j in range(len(tolerance)):
        with open(f'./one_day_logs/waterwise/time_{tolerance[j]+1}.json') as f:
            data = json.load(f)
        count1 = 0
        count2=0
        list1= []
        ...
        with open(f'./one_day_logs/waterwise/carbon_{tolerance[j]+1}.json') as f:
        ...
        with open(f'./one_day_logs/waterwise/water_{tolerance[j]+1}.json') as f:
    ...
    for index, to in enumerate(tolerance):
        with open(f'./one_day_logs/water_opt/water_{to+1}.json') as f:
            water_opt_water = json.load(f)
        with open(f'./one_day_logs/water_opt/carbon_{to+1}.json') as f:
    ...
    for index, to in enumerate(tolerance):
        with open(f'./one_day_logs/carbon_opt/water_{to+1}.json') as f:
            carbon_opt_water = json.load(f)
        with open(f'./one_day_logs/carbon_opt/carbon_{to+1}.json') as f:
    ...
    with open(f'./one_day_logs/base/carbon.json') as f:
        base_carbon = json.load(f)
    with open(f'./one_day_logs/base/water.json') as f:
    ...
    plt.savefig("./one_day_fig9.pdf",bbox_inches='tight')
    plt.show()

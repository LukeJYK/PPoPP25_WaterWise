...
class water_optimizer():
    ...
    def optimize_process(self):
        ...
        for j in tqdm.tqdm(range(self.time_length)): 
            ...
            requests,_= get_request(self.seed, request_name_extend, j,delayed_requests)

            req_exe = []
            wi_lst = []
            for region in self.regions:
                wi_lst.append(self.wi_wsf[region][j])
            ...
            for sig_req in after_sort_req:
                ...
                if best_location is None:
                    ...
                    result_carbon[j].append((best_location,np.mean(self.energy[sig_req.id])*self.ci[best_location][best_time]+self.unit_embodied_carbon*np.mean(self.exe_time[sig_req.id])))
                    result_water[j].append((best_location,np.mean(self.energy[sig_req.id])*min_intensity+self.unit_embodied_water*np.mean(self.exe_time[sig_req.id])))
        ...
        if not os.path.exists(f"{self.path}/water_opt/"):
            os.makedirs(f"{self.path}/water_opt/")
        with open(f"{self.path}/water_opt/carbon_{self.threshold+1}.json", "w") as file:
            json.dump(result_carbon, file, indent=4)
        with open(f"{self.path}/water_opt/water_{self.threshold+1}.json", "w") as file:
            json.dump(result_water, file, indent=4)
        with open(f"{self.path}/water_opt/time_{self.threshold+1}.json", "w") as file:

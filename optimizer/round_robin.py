...
class robin_optimizer():
    ...
    def optimize_process(self):
            ...
            if sum(capacity_num.values()) >= len(requests):
                for request in requests:
                    for region in self.regions:
                        if capacity_num[region] > 0:
                            sample_index = random.randint(0, 99)
                            sample_energy = self.energy[request.id][sample_index]
                            sample_exe_time = self.exe_time[request.id][sample_index]
                            result_carbon[j].append((request.seq,region,sample_energy*current_ci[region]+self.unit_embodied_carbon*sample_exe_time))
                            result_water[j].append((request.seq,region, sample_energy*current_wi_real[region]+self.unit_embodied_water*sample_exe_time))
                            result_time[j].append((
                                request.seq,
                                region,
                                np.mean(self.exe_time[request.id]),
                                sample_exe_time+(self.latency[request.id][request.src][region]+(j-request.start)*60)
                                ))
                            #update:
        ...
        if not os.path.exists(f"{self.path}/round_robin/"):
            os.makedirs(f"{self.path}/round_robin/")
        with open(f"{self.path}/round_robin/carbon.json", "w") as file:
            json.dump(result_carbon, file, indent=4)
        with open(f"{self.path}/round_robin/water.json", "w") as file:
            json.dump(result_water, file, indent=4)
        with open(f"{self.path}/round_robin/time.json", "w") as file:
